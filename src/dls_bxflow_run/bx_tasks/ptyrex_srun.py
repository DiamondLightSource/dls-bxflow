...
thing_type = BxTaskTypes.PTYREX_SRUN


class PtyrexSrun(Base):
    """ """
    ...
    def get_propagated_filename(self):
        ...
        # Use fileno to predict where ptyrex_srun_recipe writes the file.
    ...
    async def run(self):
        ...
        bash_composer.add_command(
            f"srun ptyrex_recon -c {self.__ptyrex_configfile}",
        )

        bash_filename = "%s/ptyrex_srun.sh" % (self.get_directory())
        bash_composer.write(bash_filename)

        # Build the ptyrex_srun_recipe command.
        command = []
        command.append(bash_filename)

        # These are the outputs by the various scripts leading up to the ptyrex itself.
        stdout_filename = "%s/ptyrex_srun_stdout.txt" % (self.get_directory())
        stderr_filename = "%s/ptyrex_srun_stderr.txt" % (self.get_directory())
        ...
        with open(stdout_filename, "wt") as stdout_handle:
            with open(stderr_filename, "wt") as stderr_handle:
                ...
                # TODO: Replace ptyrex_srun process.wait() with asyncio.
                process.wait(timeout=None)

                exit_code = process.returncode

        # Good return code from ptyrex_srun process itself?
        if exit_code == 0:
    ...
    def extract_error_lines(self):
        ...
        # The ptyrex_srun_recipe script normally produces a stderr file.
        # This has the "module load" blather and seems some stuff from ptyrex.
        ptyrex_srun_stderr_filename = f"{self.get_directory()}/ptyrex_srun_stderr.txt"
        ptyrex_log_filename = f"{self.get_directory()}/ptyrex.log"

        # Died before getting to ptyrex_srun?
        # For example the assertions in the workflow run before running the ptyrex_srun executable.
        if not os.path.exists(ptyrex_srun_stderr_filename):
            error_lines = [f"could not find {ptyrex_srun_stderr_filename}"]
            # Check the stdout file which is written by main.sh.
            error_lines.extend(self.extract_error_lines_from_dls_logformatter())

        elif os.path.getsize(ptyrex_srun_stderr_filename) == 0:
            error_lines = [f"nothing in {ptyrex_srun_stderr_filename}"]
            # Check the stdout file which is written by main.sh.
            error_lines.extend(self.extract_error_lines_from_dls_logformatter())
            ...
            error_lines = self.extract_error_lines_from_file(
                ptyrex_srun_stderr_filename, regexs
            )
